...
\begin{comment}
La Figure~\ref{fig:scaleadapted} illustre
...
\begin{figure}
  ...
  \caption[Spectre extrait de ZTF18accrorf avec \hypergal.]{Spectre
    ...
    qui ne sont pas utilisés pour la classification de la SN avec \pkg{SNID}.}
...
parallélisation de code. Un autre avantage de \pkg{DASK} implémenté avec
...
\pkg{DASK} permet de réduire drastiquement le temps de calcul, passant
...
\pkg{fortran}, nous utilisons un \textit{wrapper} \pkg{python}:
...
contre $z_{in}=0.042$). L'erreur sur le redshift pour un modèle avec \pkg{SNID} est
...
\begin{figure}[ht]
  ...
  \caption[Extraction de sources pour ZTF19acbjlnt.]{Extraction de
    ...
    ZTF19acbjlnt, (d) la classification \pkg{SNID} de ZTF19acbjlnt. La distance apparente entre la
...
observations, et les deux objets sont classifiés par \pkg{SNID} comme
...
avec \pkg{Dask} et utilisable aussi bien localement que via un système
